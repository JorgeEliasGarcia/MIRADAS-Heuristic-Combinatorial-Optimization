...
namespace CTSP
{
    instance::instance(void) : id_(""), distances_(), K_(0), T_(0), n_customers_(0), max_distance_(0), optimal_values_()
    {
    }
    ...
    {
        TSPLIB_instance tsplib_instance;
        ...
        K_ = tsplib_instance.get_num_days();
        ...
        for (size_t i{0}; i < n_locations; i++)
        {
            for (size_t j{0}; j < n_locations; j++)
            {
                for (size_t k{0}; k < n_locations; k++)
                {
                    if (i != j && j != k && i != k)
                    {
                        if ((distances_(i + 1, j + 1) + distances_(j + 1, k + 1)) < distances_(i + 1, k + 1))
                        {
                            return false;
                        }
                    }
